...
class NewsService {
    ...
    async search(term, page, perrPage){
       //expressa regular: comece e termine com term, 'i' eh para case sensitive true- procura em minúsculo ou maiúsculo 
        const result = await newsRepository.find({'title': new RegExp('.*'+term+'.*', 'i')}, 'title text hat')
    ...
    async get(startDate?, endDate?){
        ...
        if(startDate && endDate){
            ...
            startDate.getUTCDate()===endDate.getUTCDate() ? 
                objConsultaDate = {'active': true, 'publishDate': { $gt: `${startDate}`, $lt:endDate}} : 
                objConsultaDate = {'active': true, 'publishDate': { $gt: startDate, $lt:endDate}}

            //realiza consulta de acordo com obj montado
            return await newsRepository.find(objConsultaDate, 
                'title text hat img publishDate').sort({publishDate: -1}) // -1 tras maior para menor
               
        }else{
            // caso nao passe query com datas, retorne todos registros
            return await newsRepository.find({'active': true}, 
                'title text hat img publishDate').sort({publishDate: -1})
        }
